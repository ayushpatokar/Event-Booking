...
const Home = () => {
    ...
    return (
        <div className="flex flex-col min-h-screen">
            {/* Hero Section */}
            <div className="relative bg-black text-white rounded-3xl overflow-hidden m-12 shadow-2xl">
                <div
                ...
                {/* Added: animate-fade-in-up makes the hero content gently rise into view on page load */}
                <div className="relative p-10 md:p-20 text-center flex flex-col items-center z-10 animate-fade-in-up">
                    ...
                    <h1 className="text-5xl md:text-7xl font-black mb-6 leading-tight tracking-tight drop-shadow-lg">
                        Find Your Next <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-400">Unforgettable</span> Experience
                    </h1>
                    ...
                    {/* Added: focus-within:scale-[1.02] gives the search bar a subtle "lift" when clicked into */}
                    <div className="w-full max-w-2xl mx-auto relative flex items-center shadow-2xl group transition-transform duration-300 focus-within:scale-[1.02]">
                        ...
                        <input
                            ...
                            className="w-full pl-16 pr-6 py-5 rounded-full text-lg text-black bg-white/95 backdrop-blur-sm border-2 border-transparent focus:border-indigo-400 focus:outline-none transition-all placeholder-gray-400 font-medium"
                            value={search}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16 px-4">
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center text-center hover:-translate-y-1 transition duration-300">
                    {/* Added: gradient background on the icon circles instead of plain gray */}
                ...
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center text-center hover:-translate-y-1 transition duration-300">
                    <div className="w-16 h-16 bg-gradient-to-br from-purple-500 to-pink-600 text-white rounded-2xl flex items-center justify-center text-2xl mb-6 shadow-md shadow-purple-200/50">
                        <FaTicketAlt />
                    </div>
                ...
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center text-center hover:-translate-y-1 transition duration-300">
                    <div className="w-16 h-16 bg-gradient-to-br from-pink-500 to-rose-600 text-white rounded-2xl flex items-center justify-center text-2xl mb-6 shadow-md shadow-pink-200/50">
                        <FaShieldAlt />
                    </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {events.map(event => (
                        <div key={event._id} className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition flex flex-col group">
                            <div className="h-48 bg-gray-200 overflow-hidden relative">
                                {event.imageUrl ? (
                                    // Added: slight zoom on the image when the whole card is hovered
                                    <img
                                        src={event.imageUrl}
                                        alt={event.title}
                                    ...
                                    <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-gray-200 to-gray-300 text-gray-600 font-bold text-2xl">
                            ...
                            <div className="p-6 flex-grow flex flex-col">
                                ...
                                <div className="mt-auto">
                                    <div className="w-full bg-gray-200 rounded-full h-2 mb-2">
                                        {/* Added: gradient fill on the seats-remaining bar */}
                                        <div
                                            className="bg-gradient-to-r from-indigo-500 to-purple-500 h-2 rounded-full transition-all duration-500"
                                            style={{
                                                width: `${event.totalSeats ? (event.availableSeats / event.totalSeats) * 100 : 0}%`
                                            }}
                                        ></div>
                                    </div>
                                    <p className="text-xs text-gray-500 mb-4">{event.availableSeats} of {event.totalSeats} seats remaining</p>
                                    {/* Changed: gradient button instead of plain gray, matches hero accent colors */}
                                    <Link
                                        to={`/events/${event._id}`}
                                        className="block w-full text-center bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white font-semibold py-2 rounded-lg transition shadow-sm hover:shadow-md"
                                    >
                                        View Details
                                    </Link>
                                </div>
