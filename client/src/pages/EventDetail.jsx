...
const EventDetail = () => {
    ...
    const handleBooking = async () => {
        ...
        try {
            if (!showOTP) {
                await api.post('/bookings/send-otp', { email: user.email });
                setShowOTP(true);
    ...
    return (
        // Added: fade-in animation on page load, same as other pages
        <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden mt-8 animate-fade-in-up">
            {event.imageUrl ? (
            ...
            <div className="p-8 md:p-12">
                <div className="flex flex-col md:flex-row justify-between items-start mb-8 gap-6">
                    <div>
                        {/* Changed: category badge now uses a light indigo tint instead of plain gray, small consistent accent */}
                        <div className="inline-block bg-indigo-50 text-indigo-700 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wide mb-3">
                            {event.category}
                        </div>
                    ...
                    <div className="bg-gray-50 p-6 rounded-xl border border-gray-100 min-w-[300px] w-full md:w-auto shrink-0 shadow-sm">
                        ...
                        <div className="space-y-4 mb-8">
                            <div className="flex items-center gap-4 text-gray-600">
                                {/* Changed: icon circles now use a light indigo tint instead of gray, kept subtle */}
                        ...
                        {showOTP && (
                            <div className="mb-4">
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-indigo-500 transition shadow-sm font-bold tracking-widest text-center text-lg"
                                    value={otp}
                        ...
                        {/* Changed: main button now uses a single indigo tone instead of black, keeps one consistent accent color across the app without adding multiple gradients */}
                        <button
                            ...
                            className={`w-full py-4 px-6 rounded-xl font-bold text-lg transition shadow-lg ${isSoldOut || (successMsg && !showOTP)
                                ...
                                : 'bg-indigo-600 hover:bg-indigo-700 text-white hover:shadow-xl hover:-translate-y-1'
                                }`}
                        >
